...
* Setup a Role called `ROSADemosRole` which will be associated with OpenShift Service Account and povides the necessary authorizations as per `ROSADemosPolicy`
* Setup a IAM Policy`ROSADemosPolicy` that allows,
  ** Perform CRUD opertions on DynamoDB
  ** Perform CRUD opertions on RDS (Postgres)
